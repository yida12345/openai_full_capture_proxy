...
"""把 finalize.py 生成的 task/agent/round 数据集导出为 CyberGym 示例格式。

每条记录只包含 ``id``、``sample_type`` 和 ``messages``。工具调用使用扁平
``name/arguments``，tool 消息只包含 ``role/content``，不输出顶层工具定义。
包含 image block 的轨迹会被跳过。
...
def contains_image_block(value: Any) -> bool:
    if isinstance(value, dict):
        if value.get("type") == "image":
            return True
        return any(contains_image_block(item) for item in value.values())
    if isinstance(value, list):
        return any(contains_image_block(item) for item in value)
    return False


def segment_contains_image(segment: list[RoundRecord]) -> bool:
    last_round = segment[-1]
    request = last_round.request_body
    return any(
        contains_image_block(value)
        for value in (
            request.get("system"),
...
def build_sharegpt_record(
    ...
    history, tool_names_by_id = convert_history_messages(
        request.get("messages"), reasoning_mode, False
    )
...
def export_sharegpt(
    ...
    for task_dir in sorted(path for path in tasks_root.iterdir() if path.is_dir()):
        ...
        for agent_dir in agent_dirs:
            ...
            for segment_number, segment in enumerate(segments, 1):
                if segment_contains_image(segment):
                    continue
                file_stem = f"{agent_dir.name}_{segment_number}"
                if agent_dir.name == "main_agent":
                    sample_type = "main"
                    record_agent = "main"
                else:
                    sample_type = "subagent"
                    record_agent = agent_dir.name.replace("_", "-")
                record_id = (
                    f"{task_dir.name}-{record_agent}-segment-{segment_number:03d}"
                )
                ...
                # 与 shili/cybergym_glm51.jsonl 一样，每条轨迹单独写成多行格式化
                # JSON object；这里沿用示例的 .jsonl 扩展名。
                write_json(task_output / f"{file_stem}.jsonl", record)
                file_count += 1
    ...
    return {
        ...
        "format": "cybergym-example-flat-tool-calling",
    }


def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="把 finalize 输出的 task/agent/round 数据集转换为 CyberGym 示例格式"
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
